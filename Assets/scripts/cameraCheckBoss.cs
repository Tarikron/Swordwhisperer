...
public class cameraCheckBoss : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		switch (iState)
		{
			...
			{
				if (!camAtBoss)	
					player.GetComponent<cPlayer_c>().dialog.SendMessage("msg_eventTrigger","nearBoss",SendMessageOptions.RequireReceiver);

				if (endTimer >= startSpawn)
				{
					GameObject go = GameObject.FindGameObjectWithTag("bossSpawn");
					go.SendMessage("msg_spawnNow",null,SendMessageOptions.RequireReceiver);
				}
